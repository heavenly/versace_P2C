...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
	...
	if (g.ActiveId == id) {
		...
		if (!value_changed) {
			for (size_t i = VK_BACK; i <= VK_RMENU; i++) {
				if (io.KeysDown[i]) {
					key = i;
					value_changed = true;
					ImGui::ClearActiveID();
	...
	ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(style.Colors[ImGuiCol_TitleBg]), true, style.FrameRounding);
...
bool ImGui::ListBox(const char* label, int* current_item, std::string items[], int items_count, int height_items) {
	char** tmp = new char*[items_count];//(char**)malloc(sizeof(char*) * items_count);
