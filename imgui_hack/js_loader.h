...
public:
	...
	void execute()
	{
		const auto script_path = "C:\\VER$ACE\\scripts\\" + this->name;
		...
		this->had_error = duk_peval_string(global_vars::ctx, script.c_str()) != 0;
		if(this->had_error)
		{
			MessageBoxA(NULL, duk_safe_to_string(global_vars::ctx, -1), "js exec error", MB_OK);
		}
	}
};
