...
namespace Math
{
	...
	void Clamp(QAngle &angs)
	{
		...
		if (angs.yaw >= -180.f && angs.yaw <= 180.f)
			return;

		// get amount of rotations needed.
		float rot = std::round(std::abs(angs.yaw / 360.f));

		// normalize.
		angs.yaw = (angs.yaw < 0.f) ? angs.yaw + (360.f * rot) : angs.yaw - (360.f * rot);
	}
