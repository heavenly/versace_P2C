...
bool InputSys::ProcessMessage(UINT uMsg, WPARAM wParam)
{
	switch (uMsg) {
	...
	case WM_LBUTTONUP:
	case WM_XBUTTONUP:
		return ProcessMouseMessage(uMsg, wParam);
	case WM_KEYDOWN:
	case WM_KEYUP:
	case WM_SYSKEYDOWN:
	case WM_SYSKEYUP:
		return ProcessKeybdMessage(uMsg, wParam);
	default:
		return false;
	}
}

bool InputSys::ProcessMouseMessage(UINT uMsg, WPARAM wParam)
{
	auto key = VK_LBUTTON;
	...
	if (state == KeyState::Up && key_map[key] == KeyState::Down)
		key_map[key] = KeyState::Pressed;
	else
		key_map[key] = state;
	return true;
}

bool InputSys::ProcessKeybdMessage(UINT uMsg, WPARAM wParam)
{
	auto key = (int)wParam;
	auto state = KeyState::None;
	...
	if (state == KeyState::Up && key_map[int(key)] == KeyState::Down) {
		key_map[key] = KeyState::Pressed;
...
{
	return key_map[vk] == KeyState::Down;
}

bool InputSys::WasKeyPressed(std::uint32_t vk)
{
	if (key_map[vk] == KeyState::Pressed) {
		key_map[vk] = KeyState::Up;
		return true;
	}
	return false;
}
